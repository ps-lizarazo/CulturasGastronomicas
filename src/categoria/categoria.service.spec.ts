...
describe('CategoriaService', () => {
  ...
  it('findOne debería lanzar una excepción para una categoria inválida', async () => {
    await expect(() => service.findOne("0")).rejects.toHaveProperty("message", "No se encontro la categoria con el id suministrado")
  });
  ...
  it('create debe retornar una nueva categoria', async () => {
    ...
    const storedCategoria: CategoriaEntity = await repository.findOne({where: { id: `${nuevaCategoria.id}` }})
  ...
  it('update deberia lanzar una excepción para una categoria invalida', async () => {
    ...
    categoria = {
      ...categoria, nombre: "Condimentos"
    }
    await expect(() => service.update("0", categoria)).rejects.toHaveProperty("message", "No se encontro la categoria con el id suministrado")
  })
  ...
  it('delete deberia lanzar una excepción para una categoria invalida', async () => {
    ...
    await service.delete(categoria.id);
    await expect(() => service.delete("0")).rejects.toHaveProperty("message", "No se encontro la categoria con el id suministrado")
  });
