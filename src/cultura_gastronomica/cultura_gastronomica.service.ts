...
export class CulturaGastronomicaService {
    ...
    async findAll(): Promise<CulturaGastronomicaEntity[]> {
        return await this.culturaGastronomicaRepository.find({ relations: ["productos", "restaurantes", "recetas"] });
    }

    async findOne(id: string): Promise<CulturaGastronomicaEntity> {
        const culturaGastronomica: CulturaGastronomicaEntity = await this.culturaGastronomicaRepository.findOne({where: {id}, relations: ["productos", "restaurantes", "recetas"] } );
