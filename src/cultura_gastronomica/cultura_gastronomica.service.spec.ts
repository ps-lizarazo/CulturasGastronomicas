...
describe('CulturaGastronomicaService', () => {
  ...
  it('findOne debe retornar una cultura gastronómica por su id', async () => {
    const storedCulturaGastronomica: CulturaGastronomicaEntity = culturaGastronomicasList[0];
  ...
  it('findOne debe arrojar una excepción debido a una cultura gastronómica inexistente', async () => {
    await expect(() => service.findOne("0")).rejects.toHaveProperty("message", "La cultura gastronómica con el id brindado no ha sido encontrada.")
  });
  ...
  it('create debería crear una nueva cultura gastronómica', async () => {
    ...
    expect(storedCulturaGastronomica.descripcion).toEqual(newCulturaGastronomica.descripcion)
  });

  it('update debería actualizar una cultura gastronómica', async () => {
    const culturaGastronomica: CulturaGastronomicaEntity = culturaGastronomicasList[3];
  ...
  it('update debería arrojar error debido a una cultura gastronómica inexistente', async () => {
    let culturaGastronomica: CulturaGastronomicaEntity = culturaGastronomicasList[4];
    culturaGastronomica = {
      ...culturaGastronomica, nombre: "New name", descripcion: "New description"
    }
    await expect(() => service.update("0", culturaGastronomica)).rejects.toHaveProperty("message", "La cultura gastronómica con el id brindado no ha sido encontrada.")
  });

  it('delete debería eliminar una cultura gastronómica', async () => {
    const culturaGastronomica: CulturaGastronomicaEntity = culturaGastronomicasList[2];
  ...
  it('delete debería arrojar un error debido a una cultura gastronómica inexistente', async () => {
    const culturaGastronomica: CulturaGastronomicaEntity = culturaGastronomicasList[0];
    await service.delete(culturaGastronomica.id);
    await expect(() => service.delete("0")).rejects.toHaveProperty("message", "La cultura gastronómica con el id brindado no ha sido encontrada.")
  });
});
